...
export default function BottomsChart({ data, logScale }) {
  ...
  return (
    <>
      {data.cycles.map((c, i) => {
        ...
        const last = i === n - 1;
        return (
          <section key={c.epoch} className="bc-panel" aria-label={`Epoch ${c.epoch} cycle low`}>
            <div className="bc-hd">
              <b>Epoch {c.epoch}</b>
            ...
            <div className={'chartwrap multiple' + (last ? ' last' : '')}>
              <ResponsiveContainer width="100%" height="100%">
                {/* syncId + syncMethod="value": hovering day -120 in one panel
                    highlights day -120 in every panel, by x value rather than
                    row index (the provisional panel has fewer rows). */}
                <ComposedChart data={rows} margin={{ top: 6, right: 8, left: 0, bottom: 0 }}
                  ...
                  <XAxis dataKey="d" type="number" domain={[-data.window, data.window]} ticks={ticks}
                    hide={!last} tick={AXIS_TICK} tickFormatter={(d) => (d > 0 ? '+' : '') + d} />
      ...
      <div className="cycle-key">
        ...
        <span className="cycle-note">
          x-axis: days since each cycle low · each panel keeps its own price axis
