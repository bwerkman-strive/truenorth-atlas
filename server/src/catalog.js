...
export const METRICS = [
  ...
  // ------------------------------------------------ 2026-07 metric bundle
  {
    slug: 'op-return-fees', column: 'op_return_fees_usd', name: 'OP_RETURN Fees', category: 'network', ...usdC,
    logDefault: true,
    short: 'Fees paid by transactions that embed data via OP_RETURN, in USD.',
    explain: 'The price the data economy pays to use Bitcoin as a bulletin board: Omni and the early token era, Veriblock, and the Runes launch each printed their signature here. Against Total Fees it shows how much of the fee market is data demand rather than money movement, a split that matters for any long-run fee thesis.',
    method: 'Sum of the full fee of every transaction carrying at least one OP_RETURN output, per UTC day, valued at the day\'s close. Whole-transaction attribution, the standard convention.',
  },
  {
    slug: 'median-feerate', column: 'median_feerate', name: 'Median Fee Rate', category: 'network',
    format: 'number', unit: 'sat/vB', logDefault: true,
    short: 'The fee rate the typical transaction actually paid, in sat/vB.',
    explain: 'The honest companion to Average Fee Rate: a mean is pulled around by a handful of urgent whales, while the median is what the middle transaction really paid to get confirmed. A wide gap between the two is itself a signal, marking days when a minority of payers bid far above the going rate.',
    method: 'Per-block fee-rate histograms (fixed sat/vB buckets) merged across the day; the median transaction\'s bucket midpoint is reported, so resolution is the bucket width, not exact to the satoshi.',
  },
  {
    slug: 'active-addresses', column: 'active_addresses', name: 'Active Addresses', category: 'network', ...num,
    logDefault: true,
    short: 'Unique addresses that sent or received on-chain each day.',
    explain: 'The broadest daily participation gauge: how many distinct addresses touched the chain. Its long trend tracks adoption; its divergence from price has historically flagged rallies running without on-chain participation. Addresses are not people (one exchange address serves millions, one person can use thousands), so read the trend, not the level.',
    method: 'Exact count of distinct addresses appearing as a sender or receiver per UTC day, counted during sync (senders and receivers both; nonstandard scripts without an address are excluded).',
  },
  {
  ...
  {
    slug: 'revived-supply', column: 'revived_supply_1y', name: 'Revived Supply 1y+', category: 'behavior',
    format: 'number', unit: 'BTC', logDefault: true,
    short: 'Coins spent each day after sitting dormant for a year or more.',
    explain: 'Old coins moving is the single strongest behavioral tell on-chain: long-dormant holders act rarely, and when they do it clusters around major turning points. Sustained elevated revival during rallies is distribution by the most patient cohort; a spike during a crash is late capitulation.',
    method: 'Sum of spent-output volume with age of at least one year at spend time, per UTC day, in BTC.',
  },
  {
    slug: 'miner-outflow', column: 'miner_outflow_btc', name: 'Miner Outflow', category: 'mining',
    format: 'number', unit: 'BTC', logDefault: true,
    short: 'BTC leaving miner coinbase outputs each day.',
    explain: 'First-spends of block rewards: the moment mined coins enter circulation. Because it tracks the coinbase outputs themselves rather than guessed miner wallets, it is exact by construction; sustained outflow above the day\'s issuance means miners are drawing down treasury, classic behavior when margins compress.',
    method: 'Sum of spent outputs that were created in a coinbase transaction, per UTC day. First spends only: coins are no longer counted once they have moved from the original reward output.',
  },
  {
    slug: 'miner-unmoved-supply', column: 'miner_unmoved_supply', name: 'Miner Unmoved Supply', category: 'mining',
    format: 'number', unit: 'BTC',
    short: 'All mined coins that have never left their original reward output.',
    explain: 'The miner treasury measured with zero guesswork: coins still sitting exactly where they were mined, a large share of them Satoshi-era rewards that have never moved. Declines are miners (or ancient miners\' keys) finally spending; the series otherwise ratchets upward with every unclaimed-forever reward.',
    method: 'Sum of unspent coinbase outputs in the live UTXO set at each daily snapshot. Exact from genesis: no address clustering or entity heuristics.',
  },
  {
    slug: 'unrealized-pnl', columns: ['unrealized_profit_usd', 'unrealized_loss_usd'],
    column: 'unrealized_profit_usd', name: 'Unrealized Profit & Loss', category: 'profitloss', ...usdC,
    logDefault: true,
    short: 'Paper gains and paper losses across every coin, in dollars.',
    explain: 'NUPL nets these two against each other; here they are shown gross. Total unrealized profit is the sell-pressure reservoir (gains that could be taken), and total unrealized loss is the capitulation reservoir (pain that could be surrendered). Their relative size, not just the net, distinguishes euphoric tops from disbelief rallies.',
    method: 'For every live UTXO: value × (today\'s close − creation-day close), summed separately by sign across the daily snapshot.',
  },
  {
    slug: 'median-cost-basis', column: 'median_cost_basis', columns: ['median_cost_basis', 'price'],
    name: 'Median Cost Basis', category: 'valuation', ...usd,
    logDefault: true,
    short: 'The acquisition price of the middle coin: half of supply paid more, half paid less.',
    explain: 'Realized price is a mean, so a few coins with extreme bases can drag it; the median is the robust center of the market\'s cost structure. When spot crosses it, the literal majority of supply flips between profit and loss, which is why breaks of this level tend to matter behaviorally.',
    method: 'Weighted median of creation-day closes across the live UTXO set, interpolated within the cost-basis distribution\'s buckets (resolution is the bucket width).',
  },
  {
    slug: 'wholecoiners', column: 'wholecoiner_count', name: 'Wholecoiner Addresses', category: 'cohorts', ...num,
    short: 'Addresses holding at least one full bitcoin.',
    explain: 'The most-watched retail accumulation gauge: how many addresses could ever hold a whole coin is capped near 21 million, and the count grinding upward through bear markets has been one of the steadiest adoption signals on-chain. Addresses are not people; exchanges and custodians blur the level, so the trend carries the signal.',
    method: 'Count of addresses whose live UTXOs sum to 1 BTC or more at the daily snapshot. Address-based, not entity-based, and the copy says so on purpose.',
  },
  {
